...
##Usage
###get_url_dataset()
The function `get_url_dataset` allows downloading, unzipping and saving files with .shp and .csv extensions from `data.gov.au`. The first argument passed to function `get_url_dataset()` is the URL of the data. If file size is larger than 100 Mb and needs to be dowloaded, you can specify the second argument as `forced=TRUE` to proceed with dowloading.
